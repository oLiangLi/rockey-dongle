...
int Start(void* InOutBuf, void* ExtendBuf) {
  rlLOGI(TAG, "Start %p %p", InOutBuf, ExtendBuf);
  ...
  struct Context_t {
    ...
    int Run() {
      ...
      state[2] = 0x79622d32;
      state[3] = 0x6b206574;
      rlCryptoChaCha20Block(state, input1);
      ++state[12];
      rlCryptoChaCha20Block(state, input2);
      ++state[12];

      {
        ...
        sha256::internal_sha256_update(&ctx, input1, sizeof(input1));
        sha256::internal_sha256_final(&ctx, sha256_1);

        sha256::internal_sha256_init(&ctx);
        sha256::internal_sha256_update(&ctx, input2, sizeof(input2));
        sha256::internal_sha256_final(&ctx, sha256_2);
#else /* __RockeyARM__ */
        rlCryptoShaCtx ctx;
        rlCryptoSha256CtxInit(&ctx);
        rlCryptoSha256CtxUpdate(&ctx, input1, sizeof(input1));
        rlCryptoSha256CtxFinal(&ctx, sha256_1);

        rlCryptoSha256CtxInit(&ctx);
        rlCryptoSha256CtxUpdate(&ctx, input2, sizeof(input2));
  ...
  rlLOGXI(TAG, context, sizeof(*context), "context->Run() => %d", result);  
  std::ignore = TAG;
  return result;
}
