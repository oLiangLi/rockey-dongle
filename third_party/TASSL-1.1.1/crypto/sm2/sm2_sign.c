...
int ECDSA_sm2_get_Z(const EC_KEY *ec_key, const EVP_MD *md, const char *uid, int uid_len, unsigned char *z_buf, size_t *z_len)
{
    ...
    if (*z_len < (size_t)EVP_MD_size(md))
    {
        SM2err(SM2_F_ECDSA_SM2_GET_Z, SM2_R_INVALID_ARGUMENT);
        return 0;
    }
    ...
    if ((a == NULL) || (b == NULL))
    {
        SM2err(SM2_F_ECDSA_SM2_GET_Z, SM2_R_MALLOC_FAILURE);
        goto err;
    }
    
    if (!EC_GROUP_get_curve_GFp(group, NULL, a, b, NULL))
    {
        SM2err(SM2_F_ECDSA_SM2_GET_Z, ERR_R_EC_LIB);
        goto err;
    }
    ...
    if (!point)
    {
        ...
        if (!priv_key)
        {
            SM2err(SM2_F_ECDSA_SM2_GET_Z, SM2_R_INVALID_PRIVATE_KEY);
            goto err;
        }
...
{
    if(!eckey->priv_key)
        return eckey->meth->sign(0, dgst, dgstlen, sig, siglen, NULL, NULL, eckey);
