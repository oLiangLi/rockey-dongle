...
## 并入自上游 b360abf8: 严格警告模式(LOCAL_STRICT=0 可关; 具体警告集由
## rLANG_COMMON_STRICT_CFLAGS/CXXFLAGS 提供, 缺省为空 → 不改变现有构建)
ifneq ("$(LOCAL_STRICT)","0")
$(LOCAL_MODULE)_CFLAGS    += $(rLANG_COMMON_STRICT_CFLAGS)
$(LOCAL_MODULE)_CXXFLAGS  += $(rLANG_COMMON_STRICT_CXXFLAGS)
endif
...
## 并入自上游 b360abf8: 为该模块打开 unwind 表(宿主回溯用; LOCAL_BACKTRACE=0 可关)
ifneq ("$(LOCAL_BACKTRACE)","0")
$(LOCAL_MODULE)_CFLAGS	  += $(X4C_UNWIND_TABLE_CFLAGS)
$(LOCAL_MODULE)_CXXFLAGS  += $(X4C_UNWIND_TABLE_CFLAGS)
endif
