...
用 `tools/rockey/LIMIT/sbin/rsa-prime-repro.cjs`(与设备算法逐位对齐的独立实现)从设备上读回的种子重跑:
...
node tools/rockey/LIMIT/sbin/run-dongle-exe.cjs --tag gen1536 -2 13 3 1 10

# 读回 dashboard(自动 OpenSSL 复核并直出 seed/p/q 小端 hex)
node tools/rockey/LIMIT/sbin/run-dongle-exe.cjs --tag readkey -0 13 5

# 用种子独立复现(应得到与 dashboard 逐字节相同的素数, 且探测次数一致)
node tools/rockey/LIMIT/sbin/rsa-prime-repro.cjs --seed <seed_p> --bits 1536 --rounds 16 --expect <p>
```
