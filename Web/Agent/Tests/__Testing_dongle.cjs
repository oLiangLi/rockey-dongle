...
const Parse = require(path.join(ROOT, ".assets", "Script", "index.js")).Parse;
const EXECV =
...
function rsaEncrypt(pubkey, data) {
  return crypto.publicEncrypt({ key: pubkey, padding: crypto.constants.RSA_PKCS1_PADDING }, data);
}
...
async function List() {
  ...
  if (n <= 0 || n % 40 !== 0 || Buffer.compare(sha256(buf.subarray(0, n)), buf.subarray(n)) !== 0)
  ...
  for (let i = 0; i < n; i += 40) {
    ...
    out.push({ id, ver: info.readUInt32LE(0).toString(16), info: info.toString("base64") });
  }
  return out;
}
...
function autoParam(arg, index) {
  ...
  const fallback = () => {
    if (arg.sizeMin !== arg.sizeMax || (arg.sizeMax !== 1 && arg.sizeMax !== 2 && arg.sizeMax !== 4))
      throw Error(`param ${arg.name}: 需人工选择 (${JSON.stringify(arg.options)}), 未实现默认值`);
...
function BuildDataSegment(program, overrides) {
  ...
  for (const arg of program.data) {
    ...
    const v =
      arg.name in overrides ? overrides[arg.name] : autoParam(arg, program.data.indexOf(arg));
    if (typeof v === "number") {
      buf.writeUIntLE(v >>> 0, off, arg.sizeMax === 1 ? 1 : arg.sizeMax === 2 ? 2 : 4);
    } else {
      if (v.length < arg.sizeMin || v.length > arg.sizeMax)
        throw Error(`param ${arg.name}: size ${v.length} not in [${arg.sizeMin},${arg.sizeMax}]`);
      v.copy(buf, off);
    }
  }
...
async function ExecFrame(exe, args, frame, program) {
  ...
  for (const o of program.output || []) {
    ...
    outputs[o.name] = o.intType
      ? o.size === 4 ? chunk.readInt32LE(0) : o.size === 2 ? chunk.readInt16LE(0) : chunk.readInt8(0)
      : chunk.toString("hex");
  }
...
const EMU_EXECV =
  process.env.RKEY_EMU || path.join(ROOT, ".bin", "amd64-foobar-windows-debug", "dongle_entry.exe");
const EMU_WORLD = process.env.EMU_WORLD || path.join(ROOT, ".bin", "emu-world.bin");
...
async function EmuRun(source, bootstrap) {
  ...
  const frame = bootstrap ? FrameBootstrap(program) : await FrameNormal(program, await EmuDashboard());
...
async function EmuDiagRsa() {
  ...
  if (!fs.existsSync(EMU_WORLD)) {
    await EmuRun(fs.readFileSync(path.join(dirTests, "Initialize.dongle"), "utf8"), true);
  }
...
async function RunScript(source, hid, admin) {
  ...
  return { hid, exit: 0, outputs, stdout_tail: r.stdout.slice(line0.length).trim() };
}
...
async function main() {
  ...
  if (cmd === "dashboard") {
    ...
    const dash = await Dashboard(hid, true);
    console.log(`hid: ${hid}, dashboard 8192B sha256: ${sha256(dash).toString("hex")}`);
    return 0;
  }
  if (cmd === "run") {
    const file = argv[1];
    ...
    console.log(`run ${path.basename(file)} => ${hid}${admin ? " (admin)" : ""}`);
  ...
  if (cmd === "suite") {
    ...
    const files = fs.readdirSync(dir).filter((f) => f.endsWith(".dongle")).sort();
    if (files.length === 0) throw Error(`no .dongle in ${dir}`);
    let failed = 0;
    for (const f of files) {
      try {
        const r = await RunScript(fs.readFileSync(path.join(dir, f), "utf8"), hid, admin);
        console.log(`[PASS] ${f}`);
        if (r.outputs && Object.keys(r.outputs).length) console.log(`       ${JSON.stringify(r.outputs)}`);
      } catch (err) {
        ++failed;
        console.error(`[FAIL] ${f}: ${err.message}`);
      }
    }
    console.log(`suite ${path.basename(dir)}: ${files.length - failed}/${files.length} passed`);
    return failed === 0 ? 0 : 1;
  }
  if (cmd === "emu") {
