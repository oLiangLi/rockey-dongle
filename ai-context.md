...
- 2026-09-18 **ATOMC 侧检查接线: `atomic/ai-doc/checks/` 的三个可运行检查 → `atomic/tests/` + 统一运行器, 并挂进 `make ci`(默认跳过)**(用户裁定: "合并三个纯宿主检查 / 挂在 make ci 但默认跳过 / 检查放 atomic/tests/"):
  ①**背景(审查结论)**: `atomic/tests/{instantiate,interpreter-smoke,gate-exit-compat}.cc` 原先只在 AI 文档里手抄命令行, `git grep` 全仓 `Makefile`/`atomic/*.mk` **零命中** ⇒ `make -C atomic` 只编译、不验证任何 VM 语义; 另 `atomic/doc/isa-check.cc` 被编成 `librockey_atomic_abi_checker.a` 但**没有任何地方运行它**。
  ②**落地**: 三个 `.cc` 移到新建的 `atomic/tests/`(内容不变, 只把 `#include "rv32im-atomic.hpp"` 改成同树相对路径 `"../include/rv32im-atomic.hpp"`, 文件头注释里的旧命令行同步更新) + 新增 `atomic/tests/README.md`; 新增运行器 **`tools/rockey/ATOMC/ci/atomic-tests.cjs`**(放 `tools/rockey/<WORLD>/` 是 2026-09-14 既有分区约定) + Makefile 目标 **`make test-atomic`**。
  ③**默认跳过语义**: `run-ci.cjs` 新增 H 段 —— 只有 `CI_ATOMIC=1` 才调 runner, 否则打印"跳过 atomic(默认跳过; CI_ATOMIC=1 或 `make test-atomic` 强制执行)"。理由: `.githooks/ci-common.sh` 会在每次提交/squash merge 后自动跑 `make ci`, 不该每次都编 C++。`make test-atomic` 显式带 `--force` ⇒ 主动调用永远真跑。要改成默认执行: `atomic-tests.cjs` 的 `DEFAULT_SKIP` 置 `false`(一行)。
  ④**"环境缺失算跳过, 不算失败"(与 G1/G3 的 fail-closed 有意相反)**: runner 逐个实跑 `--version` 探测 g++/clang++/c++, 并识别"编译器起不来"的形态(`*** fatal error - CreateFileMapping … Win32 error 5.` 等 ⇒ 实测本机 Cygwin 就是如此), 输出可见 `SKIP` + 原因并返回 0; `instantiate` 用**两个**编译器各跑一次(它兼作 clang 的 `optimize` 属性探针); `interpreter-smoke`/`gate-exit-compat` 用第一个能编过的编译器编+链接+运行, 编译失败=失败(不换编译器重试)。
  ⑤**未接线(故意)**: `atomic/ai-doc/checks/varargs/`(需 `X:\Machine\ATOMIC\rv32im-atomic-rockey\bin` 交叉工具链, 是唯一"真 guest ELF 端到端"资产 —— 建议将来挂成**独立目标**)、`atomic/doc/isa-check.cc`(需要宿主板级才能运行, 设计未定)。
  ⑥**本会话验证边界(诚实记录)**: 本 harness 沙箱**禁止任何带管道的子进程**(实测 `g++`/`where.exe`/`cygwin bash` 一律 `EPERM`, 且 `.bin/` 不可写) ⇒ **SKIP 路径已实测**, 但"真编译+运行通过"**未在本会话验证**; 落地后需在真 Cygwin 终端 / WSL 侧各跑一次 `make test-atomic` 做基线。
  ⑦**顺带**: `atomic/ai-doc/README.md` 新增 §2.1(本目录其余资产索引 + 路径迁移提示), 历史文档正文里的旧路径**不做全量机械改写**(按约定不追改历史)。
- 2026-09-18 **exit 门号公式改版 (用户): 钳制 → 取模折叠 `(v & 0x7f) - 64` + CI 规则同步**:
  ①**改动**: `atomic/op_GATE/hyper/modules.cc` 的 `MatrixExit` 与 `atomic/op_GATE/hyper/hyper.h` 的 `rLANG_op_GATE_HyperExit` 各一行 —— `v < -64 ? -64 : v > 63 ? 63 : v` ⇒ **`(v & 0x7f) - 64`**。用户原话: "感觉这样更均匀一些"。
  ②**语义**: 门号 = 状态的**低 7 位** (二进制补码), 值域仍恒为 `[-64,63]`, 周期 **128** (`v` 与 `v±128` 同门); 与钳制版的关键差别 = **128 个门号全部可达且均匀命中** (钳制版只有 `-64`/`63` 两个可达、状态一越界就全挤在两个尖峰上)。代价: 门号不再携带状态的高位信息 (门号本来就只是派发槽位, 状态始终由 `a0` 完整传递)。协议其余部分 (三元组 / `kExitMagic` / 世界魔数 / `uint32` 校验和 / `kGate == gate` 校验) **一字未动**。
  ③**同步失效的断言/文档 (已全改)**: `interpreter-smoke.cc` 规程十的期望门号与文案 (钳制→折叠), 并**新增两条断言** —— 折叠周期 128 + 128 门号全可达 (`v ∈ [-4096,4096]` 逐点, 无越窗); `gate-exit-compat.cc` 的用例表重算 (`1000→40`/`-1000→-40`, `INT_MIN→-64`/`INT_MAX→63`, 并补 `64/127/128` 接缝例) + 性质块改成"值域 + 逐点一致 + 128 门号全可达 + 每槽 `64±4` 次"。
  ④**新增检查 `atomic/tests/exit-gate-map.cc`**: 专钉门号映射的规范 (值域/与无符号掩码实现逐点一致 ⇒ 负数走**低 7 位**而不是 `v % 128`/周期 128/136 个连续状态**严格均匀**/协议常量未动), 已登记进 `tools/rockey/ATOMC/ci/atomic-tests.cjs` (`--list` 现为 4 项)。它故意是**第三份公式副本**: 文件头记录了"公式在 guest/host 各写一遍、编译器看不到彼此"这个**漂移面**, 并要求改公式时同步改它; 建议 (未实施) 收成**单一来源** (如 `hyper.h` 的 C 兼容宏 `#define rLANG_EXIT_GATE_OF(v) ((int)((unsigned)(v) & 0x7fu) - 64)` 供两端调用) ⇒ 漂移面归零。
  ⑤**格式建议 (未改代码)**: `v & 0x7f` 依赖二进制补码 (GCC/clang + RV32IM 上结果正确), 更稳妥的等价写法是 `(int)((uint32_t)v & 0x7fu) - 64` (先转无符号再掩码, 标准定义)。
  ⑥**取证债**: 文档里原有一段"真机代码生成 (实测)"是**钳制版**的汇编 (`li 63 / bgt / li -64 / bge` 三条分支 + `slli`), 折叠式应为**无分支** (`andi` + `addi` 两条) —— 已改写成**按公式推出的期望形态**并标注"需重跑取证", 复核命令见 `matrix-ldscript-and-start-s-2026-09-18.md` §4.2。本会话同样**无法实测** (沙箱禁子进程, 见上一条 ⑥)。
  ⑦**计数口径更正**: 索引里"十规程 96 项断言"是历史口径; 源码现在的实际 `CHECK` 语句数是 **97 处** (规程一 12 / 二 4 / 三 3 / 四 8 / 五 3 / 六 9 / 七 2 / 八 6 / 九 33 / 十 17), 已把各文档改成"处 `CHECK`"这个可核对的口径。
- 2026-09-18 **"还是没过"的根因定位与修复 (上一轮门号改版的收尾) —— 本机找到可用的真跑路径 (MSVC)**:
  ①**用户把公式进一步改成无符号掩码写法**: `(int)((uint32_t)v & 0x7fu) - 64` (两处同步; 与我建议的稳妥写法一致), 语义与 `(v & 0x7f) - 64` **逐点等价** (已实跑核对)。
  ②**根因 = 我上一轮把 4 个用例的门号写错**: 折叠式对 `v ∈ [-64,63]` **不再是恒等映射** (`v=0 → 门 -64`、`v=63 → 门 -1`、`v=-64 → 门 0`), 而我按"区间内 状态==门号"写了 `MatrixExit(0)`(门 0)/`(42)`(门 42)/`(-64)`(门 -64)/`(63)`(门 63) ⇒ 宿主 `kGate != gate` ⇒ 这 4 个本应退出的用例返回 `SIGILL`。**实测证据**就是测试自己打印的"host 重算"值: `host 重算 -64 / -22 / 0 / -1`。已全部改成 `gateOf(v)` 计算, 并在 `interpreter-smoke.cc` 规程十、本文档 §4.2 记下这条**行为变化** (旧钳制式是恒等的, "`MatrixExit(0)` 必落 id 0"那套说辞随之作废; 安全性不变: `(*nullptr)()` 仍落 `pc=0` = id 0, 靠三元组判故障)。
  ③**顺带被这条路径抓出的两个真缺陷 (已修)**: `interpreter-smoke.cc` 里 `int wrapBad` **在同一作用域重定义** (GCC 会报重定义, 我此前没跑过所以没发现 —— 改名 `sumBad`); `__builtin_add_overflow` 是 **GCC/clang 专属** (MSVC 直接编不过) ⇒ 改成可移植的"int64 宽算 + 回代比较" `addOverflowsInt()`。
  ④**新增本地真跑路径**: `tools/rockey/ATOMC/ci/verify-atomic-checks.ps1` —— 用**本机 MSVC** (`VS2022 vcvars64` + `cl /std:c++17`) 编+链接+跑三个运行期检查, 带一个最小的 `rlLoggingWrite`/`rlLoggingWriteEx` shim (`gate-exit-compat.cc` 自带 stub, 故**不给它带 shim**, 否则 LNK2005)。为什么需要它: 本机 Cygwin 的 `g++`/`bash` 是坏的 (`CreateFileMapping` 崩溃), harness 沙箱又禁"带管道的子进程"(`spawnSync` 默认 pipe ⇒ EPERM) ⇒ node 运行器在本机两项都跑不了; MSVC 是原生进程。⚠ 它**不替代 CI** (MSVC 不认 `rLANGiOPT` 的 `optimize` 属性; `instantiate` 的意义是 g++ 与 clang++ 各看一遍)。
  ⑤**实测结果 (2026-09-18, 本机 MSVC, 工作区当前版本)**: `exit-gate-map` **9/9 全过**; `interpreter-smoke` **十规程 97 处 CHECK, failures=0**; `gate-exit-compat` **19 用例 + 性质块全过** (rc=0)。⇒ 这是这三个检查**第一次拿到真编译+真运行**的证据 (此前只有"环境缺失 ⇒ SKIP"的实测)。仍未验证: g++/clang 严格档 (`-Wall -Werror -Wshadow -Wsign-compare`)、`atomic` 交叉编译、`varargs` 端到端。
