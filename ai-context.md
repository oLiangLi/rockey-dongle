...
- 2026-09-11 修正两条约定: ①**候选边界不需要回绕保护** —— 只要构造时保证 bit1023=1 且**最高 32-bit 字 ≠ 0xFFFFFFFF**, +2 递增要到 2^1024 需 >2^990 次, 任何设备都跑不到; 原先的 2^1024-1 回绕逻辑属多余(probes 上限仅作'长时间找不到素数'的失败兜底)。②**Windows 下使用的工具脚本不加 #! shebang**(会报错), 统一 
ode xxx.cjs 调用; .bin/ukey-reset-dgram.cjs 已去掉 shebang。已同步 ai-doc/rsa-prime-bench-pitfalls-2026-09-11.md(A10/C7)与 ai-doc/rsa-root-ca-generation-2026-09-11.md(§3.2 候选约束)。
