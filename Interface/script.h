...
struct VM_t {
  ...
  /**
   *! KeyId == 10 的数据文件, 保存着由RSA2048加密的64字节主密钥(由管理员使用ComputeSecretBytes操作) ...
   */
  static constexpr int kKeyIdGlobalSECRET = 10; /* LocalChaos(RSA2048.Encrypt(MASTER_SECRET))  */

  union {
    uint16_t text_[kSizeCode];
    uint8_t text_as_buffer_[kSizeCode * 2]; /* 使用后必须立刻退出脚本运行 */
  };
...
rLANG_ABIREQUIRE(offsetof(VM_t, stack_) == offsetof(VM_t, text_) + 200);
...
enum class OpCode : uint16_t {
  ...
  kComputeSecretBytes,       /// argc : 1/2, 参数为64字节的数据, 返回结果填充这64字节数据 ...
