...
int VM_t::OpManager_UpdateMasterSecret() {
  uint8_t MASTER_SECRET[kSize_MASTER_SECRET], ENCRYPT_MASTER_SECRET[kSize_MASTER_SECRET + 96];
  int result = dongle_->ReadDataFile(Dongle::kFactoryDataFileId,
                                     WorldPublic::kOffsetDataFile + WorldPublic::kOffsetPubkey_SM2ECDSA,
                                     ENCRYPT_MASTER_SECRET, 64);
  if (0 != result)
    return result;

  result = dongle_->CheckPointOnCurveSM2(&ENCRYPT_MASTER_SECRET[0], &ENCRYPT_MASTER_SECRET[32]);
  if (0 != result)
    return result;
  ...
  result = dongle_->SM2Encrypt(&ENCRYPT_MASTER_SECRET[0], &ENCRYPT_MASTER_SECRET[32], ENCRYPT_MASTER_SECRET,
                               sizeof(ENCRYPT_MASTER_SECRET), ENCRYPT_MASTER_SECRET);
...
int VM_t::OpManager_ComputeSecretBytes(uint8_t bytes_[64]) {
  ...
  Dongle::SecretBuffer<1, Context> storage_;
  Context& MASTER_SECRET_CONTEXT = storage_[0];
  ...
  result = dongle_->SM2Decrypt(kKeyIdGlobalSM2ECDSA, MASTER_SECRET_CONTEXT.ENCRYPTO_MASTER_SECRET, size,
                               MASTER_SECRET_CONTEXT.MASTER_SECRET, kSize_MASTER_SECRET);
  if (0 != result)
    return result;

  for(int loop = 0; loop < 3; ++loop) {
    MASTER_SECRET_CONTEXT.world_seed_2_ += rLANG_WORLD_SEED_0;
    MASTER_SECRET_CONTEXT.world_seed_3_ += rLANG_WORLD_SEED_2;
    result = dongle_->SM3(&MASTER_SECRET_CONTEXT, sizeof(MASTER_SECRET_CONTEXT), &MASTER_SECRET_CONTEXT.hash_value_[0]);
    if (0 != result)
      return result;
    MASTER_SECRET_CONTEXT.world_seed_0_ += rLANG_WORLD_SEED_1;
    MASTER_SECRET_CONTEXT.world_seed_1_ += rLANG_WORLD_SEED_3;
    result =
        dongle_->SHA256(&MASTER_SECRET_CONTEXT, sizeof(MASTER_SECRET_CONTEXT), &MASTER_SECRET_CONTEXT.hash_value_[32]);
    if (0 != result)
      return result;
  }

  return dongle_->SHA512(&MASTER_SECRET_CONTEXT, sizeof(MASTER_SECRET_CONTEXT), bytes_);
}
