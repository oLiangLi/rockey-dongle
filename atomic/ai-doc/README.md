...
12. **`matrix-ldscript-and-start-s-2026-09-18.md`** (本仓自产) —— **`atomic/Matrix/ldscript/default.ld` 重写 + `start.S` 检查**。**重写后的布局** (实测段表): 三个**显式 PHDRS 权限段**, 各自 64K 页对齐 —— `.text` `R E`@`0x10000` / `.rodata` `R`@`0x20000` / `.data+.bss` `RW`@`0x30000` (bss 是 ZI: `filesz 0x10 < memsz 0x40` 不占文件); 补齐小数据段 `*(.sdata)/*(.sbss)` (原来漏了, gp 相对寻址要用)、`.data.rel.ro`/`.got`/`.tdata,.tbss`、`KEEP(*(.text.crt0))`、以及七条结构性 `ASSERT` (三段 64K 对齐 / `_estack` 16 对齐 / 不超 576K / 不重叠 / `.bss` 不落在 `.data` 之前的页)。**实测踩到两个坑并修掉**: ① `--gc-sections` 把 `.data` 回收成空段时, ld 会把 `.bss` 放回 `.text` 那一页 (RW 与 RX 共页) ⇒ 在 `.data` 里放 `LONG(0)` 保证非空 + 加断言兜底; ② 重叠断言在空段时过严 (`==` 也被判错) ⇒ 改成 `>=`。另记: 段为空时 ld 会给一个 `vaddr=0` 的零长 PT_LOAD ⇒ **宿主装载器必须跳过 size==0 的 LOAD**。**`start.S` 检查**: ✅ `gp`/`sp` 在调用 C 之前设好 (**它回答了"谁写 sp"**: crt 写, 宿主不必)、`.text.crt0` + `KEEP` 配对且实测落在映像起点、顺序正确 (先世界核对再进 `MatrixExecv`)、**符号名没问题** (`rLANGEXPORT` = `extern "C"`, 已核实宏定义); ~~两点建议~~ ⇒ **均已由用户裁定后撤回, 其中第 1 条最终以"用户修正语义"收场 (2026-09-18)**: **(1) `MatrixExecv` 现在是 `rLANGIMPORT int MatrixExecv();`** —— 它是**应用侧入口** (应用用 `rLANGEXPORT int MatrixExecv() {... return 0;}` 定义, 像 `main` 一样 return 状态; `Matrix/HelloWorld/main.cc:7`), crt 改成 **`call MatrixExecv` → `tail MatrixExit`** = `MatrixExit(MatrixExecv())` ⇒ **再没有"赌它不返回"的地方** (`call` 是正规调用, 末尾 `tail` 的目标 `MatrixExit` 由 `noreturn` 构造保证不返回, `a0` 一路原样传递; 实测反汇编确认 `a0` 未被碰) ⇒ 我原来的担心与"`call`+`ecall`"建议都作废, **用户的写法更自然**; **(2) `.bss` 归加载器** —— "**`.bss` 由加载器初始化, `start.S` 不用管**" (ELF 语义: `p_memsz > p_filesz` 时由加载器补零 ⇒ 这是**加载器契约**; 已写进 `ldscript/default.ld` 头注释**和** `start.S` 末尾块注释) ⇒ **原"crt 加 3 条清零循环"建议作废**; 仅当将来有人做 `objcopy -O binary` 平坦映像时才需另议。**`MatrixExit` 也同日修正** (只钳门号, `a0` 带完整 32 位状态) ⇒ 见下文与 `matrix-ldscript-and-start-s-2026-09-18.md` §4.2。另记一条门约定: `modules.cc` 的 guest 侧世界核对是 **高窗 hyper 门** (`4*(-512)=0xFFFFF800`, a0=worldId, a1=count, **不用 `t0`**) ⇒ 宿主 `op_GATE` 要区分 `id<0` (hyper/用户私有, 忽略 t0) 与 `[64,511]` (库导出, 必须校验 t0)。**✅ 2026-09-18 用户新增 `MatrixExit` (exit GATE) 并同日修正语义**: **只钳门号 `[-64,63]`** (`kGate = v < -64 ? -64 : v > 63 ? 63 : v` 只用于选门), **`a0` 带完整 32 位状态**; `MatrixExit(v)` 用 `4*kGate` 当函数指针调用 ⇒ `MatrixExit(0)` 落在 **id 0**, 于是"**`nullptr` 函数指针调用**"天然变成"有确定语义的一次门调用"(不再是跳到未定义地址); 负门号走高窗; 三个防误撞参数 (`a1=rLANG_CONFIG_EXIT_GATE_MAGIC`(=`0xFEA1DEAD`, hyper.h 定义, 两端**单一来源**) / `a2=v+kExitMagic` 校验和, **uint32 加 (回绕是设计)** / `a3=rLANG_WORLD_MAGIC`)。⇒ `MatrixExit` 是 `noreturn` (内部 `for(;;)` 反复调门) ⇒ **crt 现在是 `call MatrixExecv` + `tail MatrixExit`** (`MatrixExit(MatrixExecv())`): **应用入口 = `rLANGEXPORT int MatrixExecv()`** (`hyper.h` 里 `rLANGIMPORT int MatrixExecv();`), 像 `main` 一样 return 状态 ⇒ 中间是正规 `call`, 末尾 `tail` 的目标才是不返回的那个, 且 `a0` 原样传递 (实测反汇编: 两次 `auipc ra`+`jalr ra` 后 `auipc t1`+`jr t1`, **`a0` 未被碰**)。**✅ 宿主参考实现 (`hyper.h` 的 `rLANG_op_GATE_HyperExit`) 已核对为相容** (2026-09-18): 同一哨兵宏 / 同一条门号钳制表达式 (并额外核对 `kGate == gate` 派发槽位) / 同样 uint32 校验和 / 通过则 `MatrixExit(v)` 且 `v` 是**原始状态**、不匹配则 `return SIGILL` + 打日志 (fail-closed) —— 四点注意: `gate` 形参必须来自派发 (`pc/4`) 否则该检查退化为空操作; 宿主 TU **不能**定义 `rLANG_CONFIG_MATRIX_WORLD` (否则 `MatrixExit` 解析到 guest 版 ⇒ 跳到非法地址); 成功路径是宿主 `exit(v)` ⇒ **不写 `x0`、不经 `inner_Execv`**, 多世界宿主须覆盖那个 **weak** `MatrixExit`, 且 OS 只留低 8 位状态 ⇒ 要完整状态得自己记; **⚠ 但"模板在 guest 侧也编出来"是用户 2026-09-18 明确要保留的** —— 为将来的**世界嵌套** ("VMX.Linux 里启动 VMX.DOS 里启动 VMS.NES"): 同一构建里一个世界既是外层 guest 又是内层 host ⇒ 于是中间层的成功路径不能 `exit(v)` (会连最外层一起杀掉), 且**状态 0 走不了 `x0`** (`err` 就是 `zero`), 需要"状态槽 + 专用完成码 `0xC8C04E1E`" (待定, 见 `matrix-ldscript-and-start-s-2026-09-18.md` §4.2 第 4 点)。 **用户 2026-09-18 两条裁定**: **`.bss` 由加载器初始化** (ELF 语义 `p_memsz > p_filesz` 补零 ⇒ 这是**加载器契约**, `start.S` 保持原样; 已写进 `ldscript/default.ld` 注释**和** `start.S` 末尾块注释; 顺带实测 `start.S` 汇编 rc=0 且确认 `tail` ⇒ `jr t1` **不写 `ra`**) 与 "**`exit(0)` 与真正的 `(*nullptr)()` 状态有差别, `MatrixExit` 会设置几个 magic number**" (⇒ 两者同 id 但参数不同 ⇒ **只有"只看 id"的实现才会混淆**; 宿主在 `id ∈ [-64,63]` 上必须分支: 三元组匹配 ⇒ 正常退出并记 `a0` 状态, 不匹配 ⇒ 按故障处理/至少告警) —— ✅ **判别规则已落成可运行断言**: `checks/interpreter-smoke.cc` **规程十** (14 项, 全套十规程 94 项 0 失败), 见本页第 7 条。另: `%n` 策略仍未定 (HelloWorld 刻意用 `%n`+`%zd` 在测宿主的格式化器)。

## 2.1 本目录的其余资产 (2026-09-18 补记; 上面的阅读顺序之外)

> ⚠ **路径更新 (2026-09-18)**: 下面三个**可运行检查** (`instantiate.cc` / `interpreter-smoke.cc` /
> `gate-exit-compat.cc`) 已从本目录的 `checks/` 移到 **`atomic/tests/`**, 并接上统一运行器
> **`tools/rockey/ATOMC/ci/atomic-tests.cjs`** 与 Makefile 目标 **`make test-atomic`**
> (默认不跑; `make ci` 里默认跳过, `CI_ATOMIC=1` 才执行)。本目录里的**文档**正文保留旧路径不动
> (按既定约定不追改历史文档), 以本表与 `atomic/tests/README.md` 为准。
> 仍留在 `checks/` 下的只有 `varargs/` (需要 rv32im 交叉工具链, 尚未接线)。

| 文件 | 性质 | 一句话 |
| --- | --- | --- |
| `matrix-ldscript-and-start-s-2026-09-18.md` | 本仓自产 | `Matrix/ldscript/default.ld` 重写 (RX/R/RW 三个 64K 对齐权限段 + 7 条 ASSERT) + `start.S` 检查 + **exit GATE 协议** (`[-64,63]`, `call MatrixExecv → tail MatrixExit`, 三元组判别) ⇒ §4.2 是宿主侧契约清单 |
| `../tests/instantiate.cc` | 可运行 | 一行显式实例化 ⇒ 逼编译器检查 `VM_t` 全部成员函数体 (反面对照: 注回旧写法立刻被抓 117 个错误) |
| `../tests/interpreter-smoke.cc` | 可运行 | **十规程 / 97 处 `CHECK`**: 算术/访存/跳转/门/`zero`/`TIMEDOUT` + 宿主 API + 两版内存布局 + hart 初值契约 + 周期计价 + **exit GATE 判别** (含 128 门号全可达/均匀) |
| `../tests/gate-exit-compat.cc` | 可运行 | exit 门**两端相容**: 18 用例 + 门号折叠的性质 (值域/逐点一致/128 门号全可达且均匀) + 有符号 UB 反面断言 |
| `../tests/exit-gate-map.cc` | 可运行 | exit **门号映射的规范** (2026-09-18 改版: 钳制 → 取模折叠 `(v & 0x7f) - 64`): 值域 / 周期 128 / 128 门号全可达 / 严格均匀 / 负数走低 7 位 |
| `checks/varargs/` | 可运行 | 变参/调用约定端到端: guest 程序 + ELF 装载 runner, 在真 `VM_t` 上跑通 (VM 一行未改); `guest-fmt.c` 待接线; **需 rv32im 工具链, 尚未接进运行器** |
| `op-gate-list-yaml-review-2026-09-17.md` | 本仓自产 | 调用门清单格式评审 (用户八条裁定) + `op_GATE/tools/gen.cjs` 生成器实测 |
| `musl-rv32im-build-atomics-2026-09-17.md` | 本仓自产 | `make -j8` 曾 rc=2 的**唯一**根因 (`a_cas` 需要 A) 与三条路径; 已修 ⇒ 0 错误、三库无 `lr.w/sc.w/amo*` |
| `atomic/doc/riscv-spec.pdf` | 离线依据 | RISC-V 卷 I 20260120 (Ratified); RV32IM 用户态语义可离线核对, trap/psABI **不在**其中 |

**当前代码骨架 (2026-09-18 13:49 状态, 仓干净, HEAD `d8bf67e`)**: `atomic/include/rv32im-atomic.hpp` (656 行, `hyper::VM_t`, 全部钩子默认 fail-closed) / `atomic/op_GATE/{list,gen,hyper,helper,tools}` / `atomic/Matrix/{HelloWorld,ldscript}` / **`atomic/tests/` (宿主侧检查 + 自己的 README)** / `atomic/musl` 覆盖层 / `atomic/project.mk` (`-I` 顺序: 覆盖层必须在 `third_party/musl/arch/riscv32` **之前**) / `atomic/toolchain/rv32im-atomic-rockey.conf` (显式钉 `-march=rv32im -mabi=ilp32`)。
**检查的运行入口 (2026-09-18 新增)**: `tools/rockey/ATOMC/ci/atomic-tests.cjs` + `make test-atomic` (默认不跑; `make ci` 里默认跳过, `CI_ATOMIC=1` 才执行)。
工具链实测存在: `X:\Machine\ATOMIC\rv32im-atomic-rockey\bin` (gcc 16.1.0, `riscv32-unknown-elf-*`)。
