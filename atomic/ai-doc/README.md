...
- **待确认的细节 (可能与上述"填随机数"是同一件事)**: 头文件判据 (`pc < 0x00000800 || pc >= 0xFFFFF800`) 比 `atomic/README.md` 第 18 行分配的窗口 (`0xFFFF'F800–0xFFFF'FF00`、`0x0000'0100–0x0000'07FF`) **更宽**: 多出 `0x000–0x0FF` 与 `0xFFFF'FF00–0xFFFF'FFFF` 两段。按 `id = (int)pc / 4`, 低窗 `id = 0…511` (多出的那段是 `id 0…63`)、高窗因 `(int)0xFFFFF800 < 0` 得**负 id** (`-512…-1`, 多出的那段是 `id -64…-1`)。**这两段多出来的空间正好可以放"每次编译都变的随机数"** —— 若确实如此, 则判据比窗口更宽是**刻意为之**, 无需收窄; 请确认随机填充的范围是否就落在这两段 (抑或也包含"未实现的门槽位")。`atomic/README.md` 属维护者文件, 我未改动。
